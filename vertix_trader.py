...
def create_target(df):
    """Creates the target variable for the next 3 minutes."""
    df['future_close_3m'] = df['Close'].shift(-3)  # Change from -5 to -3
    df['price_change_3m'] = (df['future_close_3m'] - df['Close']) / df['Close']

    df['target'] = np.nan
    df.loc[df['price_change_3m'] > PROFIT_THRESHOLD_PC, 'target'] = 1  # LONG
    df.loc[df['price_change_3m'] < -PROFIT_THRESHOLD_PC, 'target'] = 0 # SHORT
    ...
    # Drop future-looking columns before returning
    df.drop(columns=['future_close_3m', 'price_change_3m'], inplace=True, errors='ignore')
    return df
...
class TradingPredictor:
    ...
    def save_prediction_to_firestore(self,predicted_price, prediction, timestamp, actual_price=None, is_correct=None, outcome="NEUTRAL"):
        try:
